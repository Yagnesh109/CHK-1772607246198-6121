...
class SecureStoreService {
  static final String _host =
      dotenv.env['API_HOST'] ?? "backend-medicine-app-sveri-hackathon.onrender.com";
  static final bool _useHttps =
      (dotenv.env['API_SCHEME'] ?? 'https').toLowerCase() == 'https';
  static final int? _port = int.tryParse(dotenv.env['API_PORT'] ?? '');
  static String? _cachedRole;

  static String? getCachedRole() => _cachedRole;

  static void setCachedRole(String? role) {
    _cachedRole = role?.trim().isEmpty ?? true ? null : role?.trim();
  }
  ...
  static Future<Map<String, dynamic>> getUserProfile({
    ...
    // Fast path: return cached role immediately to avoid blocking UI. Caller can
    // opt into a refresh by setting forceRefresh=true.
    if (!forceRefresh && _cachedRole != null && _cachedRole!.isNotEmpty) {
      return {"role": _cachedRole, "fromCache": true};
    }

    final res = await _authorizedGet("/secure/user/profile");
    if (res['error'] == null) {
      setCachedRole(res['role']?.toString());
    }
    return res;
  }

  static Future<Map<String, dynamic>> setUserRole(String role) {
    return _authorizedPost("/secure/user/role", {"role": role}).then((res) {
      // Keep local cache in sync so UI updates immediately after switching.
      if (res['error'] == null) {
        setCachedRole(role);
      }
      return res;
